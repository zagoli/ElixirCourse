defmodule PentoWeb.GameLive.Component do
  ...
  defp place_pento({name, idx}) do
    Pentomino.new(name: name, location: location(idx))
    |> Pentomino.to_shape()
  end

  defp location(idx) do
    x = rem(idx, 6) * 4 + 3
    y = div(idx, 6) * 5 + 3
    {x, y}
  end
  ...
  def triangle(assigns) do
    ~H"""
    <use xlink:href="#triangle" x={@x} y={@y} fill={@fill} transform={"rotate(#{@rotate},10,10)"} />
    """
  end
end
